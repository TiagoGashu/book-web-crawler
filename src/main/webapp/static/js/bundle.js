...
store = new Vuex.Store({
  ...
  mutations: {
    updateLivros(state, responseJson) {
      ...
      this.state.livros.splice(0, this.state.livros.length);
 ...
 * Declarando os componentes
 ...
 var ItensComp = Vue.extend({ 
   ...
   template: '<ul>' + 
    ...
    '                     <big>{{ livro.name }}</big> - <small>{{ livro.authorName }}</small>' + 
    ...
    methods: {
      refreshLivros() {
        store.dispatch('refreshLivros').then(() => {
          debugger;
        })
      }
    },
   ...
   methods: { 
     addLivro: function () { 
       ...
       if (titulo) { 
    	 ...
    	 axios.post('http://localhost:8080/books', novoLivro).then(function(response) {
 ...
 var RemoveItensComp = Vue.extend({
	data: function() {
		return store.state;
	},
	methods: {
		removeBooks: function() {
			var ids = [];
			store.state.livros.forEach(function(livro) {
				if(livro.checked) {
					ids.push(livro.id);
				}
			})
	...
	template:
		'<div class="row">' +
		'    <button@click="removeBooks" class="js-remove btn btn-primary offset-md-4s"' +
 ...
 Vue.component('remove-itens-comp', RemoveItensComp);
 ...
 // inde.js FIM
},{"axios":3,"vue":29,"vuex":30}],3:[function(require,module,exports){
